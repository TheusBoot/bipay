...
class HackALappHTTPServer_RequestHandler(BaseHTTPRequestHandler):
    ...
    # Inicializando a interface RPC para comunicação com c-lightning
    rpc_interface = LightningRpc("/home/rpickhardt/.lightning/lightning-rpc") 
    
    # Carregando os tópicos de um arquivo e armazenando-os em um dicionário
    f = open("topics.txt")
    topic_dict= {}
    for line in f:
    ...
    # Método para gerar o rodapé da página HTML
    def __footer(self):
    ...
    # Método para gerar um código QR base64 a partir de um texto
    ...
    # Método para adicionar uma string de pagamento de fatura a uma mensagem HTML
    ...
    # Método para contar os votos registrados
    ...
    # Método para exibir a página inicial de votação
    ...
    # Método para exibir a página de classificação dos votos
    def __show_rankings_page(self,payment_hash):
        ...
        """
        CODING --> message += self.__add__invoice_payment_string(invoice)

        Essa linha de código está concatenando à variável `message` o resultado da chamada de uma função chamada `__add__invoice_payment_string()`, passando o parâmetro `invoice` para ela.

        A função `__add__invoice_payment_string()` provavelmente é responsável por gerar uma mensagem HTML que contém informações sobre um pagamento de fatura (invoice) no contexto do código fornecido. Isso pode incluir detalhes como o custo da fatura, o código de pagamento (bolt11), e possivelmente até mesmo uma representação visual do código de pagamento, como um QR code.

        A função provavelmente é usada para criar uma mensagem HTML que será exibida na página web gerada pelo servidor HTTP, mostrando informações relevantes sobre a fatura que está sendo processada ou sobre o pagamento que está sendo solicitado.
        """
                
        return message

    # Método chamado para lidar com solicitações GET
    def do_GET(self):
    ...
    # Método chamado para lidar com solicitações POST
    def do_POST(self):
        self.send_response(200)
        self.send_header('Content-type', 'text/html')
        self.end_headers()
